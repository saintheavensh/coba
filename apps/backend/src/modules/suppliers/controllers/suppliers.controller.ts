...
export class SuppliersController {
    ...
    async create(c: Context) {
        try {
            ...
            // Check if it's a validation error (Zod usually has .issues or .errors)
            // Or if we specifically caught the mock error for validation
    ...
    async delete(c: Context) {
        try {
            ...
            return apiError(c, e, "Failed to delete supplier", 400); // Original code had 400 here, kept it or should be 500? Use 500 for consistency unless default delete is 400
        }
    }
