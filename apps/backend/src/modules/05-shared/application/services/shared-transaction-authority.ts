 ...
 *
 * SharedTransactionAuthority — Single entry point for all 05-shared database operations.
 *
 * All shared code MUST route through either:
...
export class SharedTransactionAuthority {
    ...
    private async _runWithLogging<T>(
        ...
        if (typeof context.tenantId !== "string" || context.tenantId.trim().length === 0) {
            throw new Error("SharedTransactionAuthority: tenantId is strictly mandatory (non-empty string) for all operations.");
        }

        const rid = context.requestId || randomUUID();
        const startedAt = Date.now();
        const logContext: LogContext = { service: "shared", tenantId: context.tenantId, requestId: rid };
    ...
    private validateTxTenant(tx: TransactionContext, tenantId: string): void {
        ...
        if (!txTenantId) {
            throw new Error("SharedTransactionAuthority: tx missing tenantId metadata. All transactions must be created through authority.");
        }
        if (txTenantId !== tenantId) {
            throw new Error(
                `SharedTransactionAuthority: tenant mismatch. tx.tenantId="${txTenantId}" but caller tenantId="${tenantId}". Cross-tenant operations are forbidden.`
            );
        }
    }
    ...
    async execute<T>(
        ...
        const rid = context.requestId || randomUUID();
        const fullContext: AuthorityContext = { ...context, requestId: rid };

        return this.db.transaction(async (baseTx) => {
            const tx = this.enrichTx(baseTx as unknown as TransactionContext, fullContext, rid);
