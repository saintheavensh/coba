import { db } from "../../../db";
import { accounts, journalLines, journals } from "../../../db/schema";
...
export class AccountsService {
    ...
    static async getAccountTypes() {
        return AccountModel.findTypes();
    }

    /**
     * Get all accounts (flat list)
     */
    static async getAll(filters: { typeId?: string } = {}) {
        return AccountModel.findAll(filters);
    }
    ...
    static async create(input: CreateAccountInput, userId?: string): Promise<string> {
        ...
        const type = await AccountModel.findTypeById(input.typeId);
    ...
    static async recalculateBalance(accountId: string): Promise<number> {
        ...
        await AccountModel.update(accountId, { balance, updatedAt: new Date() });
     ...
     * Creates a journal entry: Debit/Credit Account <-> Credit/Debit Opening Balance Equity
     */
    static async setOpeningBalance(accountId: string, amount: number, userId?: string, date: Date = new Date()) {
        const account = await this.getById(accountId);
        ...
        // We'll look for standard 3-xxxx Equity account for opening balance
        // If not found, create one.
        const equityCode = "3-9000"; // Reserved for Opening Balance
        let equityAccount = await AccountModel.findByCode(equityCode);
        ...
        if (!equityAccount) {
            ...
            await this.create({
                ...
                typeId: "EQUITY",
                description: "System account for initial balances" // Fixed: removed invalid property 'isSystem'
            });
            equityAccount = await AccountModel.findByCode(equityCode);
        }

        if (!equityAccount) throw new Error("Failed to resolve Opening Balance Equity account");

        // 2. Determine Dr/Cr based on Account Type & Amount
        // Asset/Expense: Increase = Debit. So we Debit Account, Credit Equity.
        // Liab/Equity/Revenue: Increase = Credit. So we Credit Account, Debit Equity.
        ...
        // 3. Create Journal
        // Check if there's already an opening balance journal for this account? 
        // For simplicity, we just post a new one. The User UI should handle "Edit" by Voiding previous if needed, 
        // or we just allow incremental additions. 
        // The prompt says "Input known values". 

        await db.transaction(async (tx) => {
            // Create Journal Header
            const journalId = `JRN-OB-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
            ...
            await tx.insert(journalLines).values({
                ...
                debit: 0,
                credit: amount,
                description: `Offset Opening Balance`
            });
        });

        // Recalculate balances
        await this.recalculateBalance(account.id);
        await this.recalculateBalance(equityAccount.id);
    }
