...
describe("assertProductStockConsistency", () => {
    it("does not throw when product.stock equals sum(batches.current_stock)", async () => {
        ...
        await expect(assertProductStockConsistency([productId], mockTx)).resolves.toBeUndefined();
    });
    ...
    it("throws when product.stock does not equal sum(batches.current_stock)", async () => {
        ...
        await expect(assertProductStockConsistency([productId], mockTx))
    ...
    it("checks all given product ids", async () => {
        ...
        await expect(assertProductStockConsistency(["P1", "P2"], mockTx)).resolves.toBeUndefined();
