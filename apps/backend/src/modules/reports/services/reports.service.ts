import { db } from "../../../db";
import { sales, saleItems, productBatches, services, purchases, users, activityLogs, operationalCosts, categories, products } from "../../../db/schema";
...
export class ReportsService {
     ...
     */
    async getSalesSummary(filters: ReportFilters = {}, dbOrTx?: any): Promise<SalesSummary> {
        let conditions = [];
        ...
        const salesData = await this.model.getSalesData(conditions, dbOrTx);
     ...
     */
    async getTransactions(filters: ReportFilters = {}, dbOrTx?: any): Promise<TransactionReport[]> {
        let conditions = [];
        ...
        const salesData = await this.model.getTransactions(conditions, dbOrTx);
     ...
     */
    async getServiceStats(filters: ReportFilters = {}, dbOrTx?: any) {
        let conditions = [];
        ...
        const servicesData = await this.model.getServices(conditions, dbOrTx);
     ...
     */
    async getServiceTransactions(filters: ReportFilters = {}, dbOrTx?: any): Promise<ServiceReport[]> {
        let conditions = [];
        ...
        const servicesData = conditions.length > 0
            ? await effectiveDb.query.services.findMany({
     ...
     */
    async getPurchasesSummary(filters: ReportFilters = {}, dbOrTx?: any): Promise<PurchasesSummary> {
        let conditions = [];
        ...
        const purchasesData = await this.model.getPurchases(conditions, dbOrTx);
     ...
     */
    async getPurchaseTransactions(filters: ReportFilters = {}, dbOrTx?: any): Promise<PurchaseReport[]> {
        let conditions = [];
     ...
     */
    async getTechnicianStats(filters: ReportFilters = {}, dbOrTx?: any): Promise<TechnicianReport[]> {
        let conditions = [];
        ...
        const technicians = await this.model.getTechnicians(dbOrTx);

        // Get all services in date range
        const servicesData = await this.model.getServicesWithTechnicians(conditions, dbOrTx);
     ...
     */
    async getPartsUsageReport(filters: ReportFilters = {}, dbOrTx?: any): Promise<PartsUsageReport[]> {
        let conditions = [];
     ...
     */
    async getActivityLogs(filters: ReportFilters & { userId?: string; action?: string; entityType?: string; limit?: number } = {}, dbOrTx?: any) {
        let conditions = [];
        ...
        const logs = await this.model.getActivityLogs(conditions, filters.limit, dbOrTx);
     ...
     */
    async getProfitAndLoss(filters: ReportFilters = {}, dbOrTx?: any): Promise<ProfitAndLoss> {
        let salesConditions = [];
        ...
        const salesData = await this.model.getSalesData(salesConditions, dbOrTx);
        ...
        const servicesData = await this.model.getServicesWithTechnicians([
        ...
        const expensesData = await this.model.getOperationalCosts(expenseConditions, dbOrTx);
     ...
     */
    async getStockValueReport(dbOrTx?: any): Promise<StockValueReport> {
        const categoriesData = await this.model.getCategoriesWithStock(dbOrTx);
