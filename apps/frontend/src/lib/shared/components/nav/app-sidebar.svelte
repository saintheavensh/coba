<script lang="ts">
  ...
  let userName = $derived(authStore.user?.name || null);
  let userRole = $derived(authStore.role);
  let roleDisplayName = $derived.by(() => {
    ...
    return map[userRole as string] || userRole || "Online";
  });
  ...
  let filteredMenuGroups = $derived(() => {
    let groups = JSON.parse(JSON.stringify(menuGroups)); // Deep copy to avoid mutating original for icons? Wait icon is a component.
    // Actually icons are components, JSON.parse will kill them. Let's just filter manually.

    // Better way: Filter existing structure
    return menuGroups
      .map((group) => {
        const filteredItems = group.items.filter((item) => {
          ...
          // Owner - Strategic Oversight (Analytics Only)
          if (authStore.hasRole("owner")) {
            if (
              ["Dashboard", "Laporan", "Akuntansi", "Stock Check"].includes(
                item.title,
              )
            ) {
              return true;
          ...
          if (authStore.hasRole("manager")) {
            if (
              ...
              if (item.title === "Setting" && item.children) {
                // Admin only sees Employees and maybe Store Info in settings
                ...
                // Also hide highly sensitive "Factory Reset" and "Integration"
              }
              return true;
            }
          }
          ...
          // Technician
          if (authStore.hasRole("teknisi")) {
            if (
              ["Dashboard", "Service", "Alat Service", "Garansi"].includes(
                item.title,
              )
            )
              return true;
          ...
          // Cashier
          if (authStore.hasRole("kasir")) {
            if (
              ["Dashboard", "Penjualan", "Customers", "Service"].includes(
                item.title,
              )
            )
              return true;
          ...
          // Warehouse
          if (authStore.hasRole("warehouse")) {
            if (
              [
                "Dashboard",
                "Produk",
        ...
        const prefix = userRole === "super_admin" ? "/superadmin" : "";
        const prefixedItems = filteredItems.map((item) => {
          const newItem = { ...item };
          if (newItem.href && newItem.href.startsWith("/")) {
            newItem.href = prefix + (newItem.href === "/" ? "" : newItem.href);
          }
          if (newItem.children) {
            newItem.children = newItem.children.map((child) => {
              const newChild = { ...child };
              if (newChild.href && newChild.href.startsWith("/")) {
                newChild.href =
                  prefix + (newChild.href === "/" ? "" : newChild.href);
              }
              if (newChild.children) {
                newChild.children = newChild.children.map((subChild) => {
                  const newSubChild = { ...subChild };
                  if (newSubChild.href && newSubChild.href.startsWith("/")) {
                    newSubChild.href =
                      prefix +
                      (newSubChild.href === "/" ? "" : newSubChild.href);
                  }
                  return newSubChild;
                });
              }
        ...
        return { ...group, items: prefixedItems };
      })
        ...
        if (item.children) {
          ...
          for (const child of item.children) {
            if (child.children) {
              ...
              (pathOnly === child.href ||
                (child.href !== "/" &&
                  child.href !== "/superadmin" &&
                  pathOnly.startsWith(child.href)))
            ) {
              hasActiveChild = true;
            }
          }
  ...
  import { uiStore } from "$lib/shared/components/nav/ui-store.svelte";
      ...
      <div class="flex items-center gap-3 px-2 mb-2" transition:slide|local>
        ...
        <div class="flex flex-col overflow-hidden">
          <span class="text-sm font-medium text-white truncate"
            >{userName || "User"}</span
          >
          <span class="text-xs text-slate-500 truncate">{roleDisplayName}</span>
        </div>
      </div>
    {/if}
