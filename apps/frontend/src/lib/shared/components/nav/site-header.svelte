<script lang="ts">
    ...
    import { activityLogs } from "$lib/features/settings/settings-store.svelte";
    ...
    let userName = $derived(
        authStore.user?.name || authStore.user?.username || null,
    );
    let userRole = $derived(authStore.role);
    let userEmail = $derived(
        authStore.user?.email ||
            (authStore.user?.username
                ? `${authStore.user?.username}@example.com`
                : null),
    );

    onMount(() => {
        // No longer need to fetch from localStorage
    });

    // Role-based filtering for mobile menu
            ...
            .map((group) => {
                ...
                const prefixedItems = filteredItems.map((item) => {
                    ...
                    if (newItem.href && newItem.href.startsWith("/")) {
                        newItem.href =
                            prefix + (newItem.href === "/" ? "" : newItem.href);
                    }
                    if (newItem.children) {
