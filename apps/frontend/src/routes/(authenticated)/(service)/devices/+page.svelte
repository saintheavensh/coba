<script lang="ts">
    ...
    import { BrandsService } from "$lib/features/inventory/services/brands.service";
    ...
    const handleSync = async (deviceId: string) => {
        try {
            isSyncing = true;
            toast.loading("Syncing compatibility...", { id: "sync-toast" });
            const res =
                await InventoryService.syncDeviceCompatibility(deviceId);
            toast.dismiss("sync-toast");
            if (res.count > 0) {
                toast.success(`Synced! Linked ${res.count} products.`);
            } else {
                toast.info("Sync complete. No new matches found.");
            }
            // Refetch to update any compatibility counts if we display them (optional)
            queryClient.invalidateQueries({ queryKey: ["devices"] });
        } catch (e) {
            toast.dismiss("sync-toast");
            console.error(e);
            toast.error("Failed to sync device compatibility.");
        } finally {
            isSyncing = false;
        }
    };
            ...
            <div class="flex items-center gap-2">
                {#if selectedIds.length === 0}
                    <Button
                        variant="outline"
                        size="lg"
                        class="h-11 rounded-xl bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800"
                        href="/devices/unlinked"
                    >
                        <PackageX class="h-4 w-4 mr-2 text-slate-500" />
                        Unlinked
                    </Button>
