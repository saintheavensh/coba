<script lang="ts">
    ...
    async function loadData() {
        try {
            ...
            if (authStore.role === "teknisi") {
                tools = await ServiceToolsService.getMyTools();
                requests = await ServiceToolsService.getMyRequests();
            } else {
    ...
    const isAdmin = $derived(authStore.hasRole(["admin", "owner"]));
    const isTeknisi = $derived(authStore.hasRole("teknisi"));
    ...
    async function updateCondition(
        ...
        try {
            await ServiceToolsService.updateCondition(toolId, condition);
            toast.success("Kondisi alat diperbarui");
            loadData();
        } catch (error) {
    ...
    async function submitRequest() {
        ...
        try {
            ...
            toast.success("Permintaan alat dikirim");
...
<div class="min-h-screen space-y-8 p-6 pb-20">
        ...
        >
            <div>
                ...
                <p class="text-blue-100 max-w-xl text-lg">
                    {isTeknisi
                        ? "Peralatan service yang diberikan oleh pemilik toko."
    ...
    >
            ...
            >
                <Card.Content class="p-5">
                    <div class="flex justify-between items-start">
                        ...
                        <div
                            class="p-2 bg-red-100 dark:bg-red-900/20 rounded-lg"
                        >
        ...
        {:else if isTeknisi}
            <Card.Root
                class="bg-card/50 backdrop-blur border-l-4 border-l-orange-500 shadow-lg hover:shadow-xl transition-all"
            >
                <Card.Content class="p-5">
                    <div class="flex justify-between items-start">
                        <div>
                        ...
                        <div
                            class="p-2 bg-orange-100 dark:bg-orange-900/20 rounded-lg"
                        >
    ...
    <div in:fade={{ duration: 500, delay: 300 }} class="grid gap-6">
                                            ...
                                            <div class="flex gap-1">
                                                {#if tool.condition !== "damaged"}
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        class="h-8 text-xs hover:bg-red-100 hover:text-red-600"
                                                        onclick={() =>
                                                            updateCondition(
                                                                tool.id,
                                                                "damaged",
                                                ...
                                                {/if}
                                                {#if tool.condition !== "lost"}
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        class="h-8 text-xs hover:bg-amber-100 hover:text-amber-600"
                                                        onclick={() =>
                                                            updateCondition(
                                                                tool.id,
                                                                "lost",
                                                ...
                                                {#if tool.condition !== "good"}
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        class="h-8 text-xs hover:bg-emerald-100 hover:text-emerald-600"
                                                        onclick={() =>
                                                            updateCondition(
                                                                tool.id,
                                                                "good",
        ...
        {#if isTeknisi && requests.length > 0}
            <div class="space-y-4 mt-8">
                <div class="flex items-center gap-2">
                    <History class="h-5 w-5 text-muted-foreground" />
                    <h2 class="text-xl font-bold">Riwayat Permintaan Alat</h2>
                </div>
                <div
                            ...
                            {#each requests as request}
                                <TableRow>
                                    <TableCell class="font-medium"
                                        >{request.toolName}</TableCell
                                    >
                                    <TableCell
                                        class="text-muted-foreground text-sm"
                                        >{request.justification}</TableCell
                                    >
...
<!-- Request Tool Dialog -->
<Dialog bind:open={isRequestDialogOpen}>
    <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
            ...
            <p class="text-sm text-muted-foreground">
                Ajukan permintaan peralatan yang Anda butuhkan untuk mendukung
                pekerjaan.
            </p>
        </DialogHeader>
        <div class="space-y-4 py-4">
            <div class="space-y-2">
                ...
                <Input
                    ...
                    placeholder="Contoh: Solder Station T12-D"
                />
            </div>
            <div class="space-y-2">
                <Label for="justification">Alasan / Justifikasi</Label>
                <Textarea
                    id="justification"
                    bind:value={requestForm.justification}
                    placeholder="Kenapa Anda membutuhkan alat ini?"
                    class="h-32"
                />
            </div>
        </div>
        <DialogFooter>
            <Button
                variant="outline"
                onclick={() => (isRequestDialogOpen = false)}>Batal</Button
            >
